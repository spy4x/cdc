<script lang="ts">
  import { type RequestError, VALIDATION_ERROR_CODE, type ValidationError } from '@shared';
  ...
  $: validationError = error && error.code === VALIDATION_ERROR_CODE && field && error.errors[field];
</script>
